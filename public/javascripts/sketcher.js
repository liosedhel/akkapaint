...
Sketcher.prototype.updateCanvasByBrush = function (event) {
	...
	for ( var z=0; (z<=distance || z==0); z++ )
	{
        ...
        var pixel_list = [];
        for (var i = 0; i < this.brush.width; i++) {
		    for (var j = 0; j < this.brush.height; j++) {
                pixel_list.push(x + i);
				pixel_list.push(y + j)
            }
		}
        exampleSocket.send(JSON.stringify({"pixels": pixel_list, "color": this.color}));

        this.simpleDraw(x, y, this.color, this.brush.width, this.brush.height);
	}

}

Sketcher.prototype.updateCanvasFromServer = function (x, y, color) {
    this.simpleDraw(x, y, color, 1, 1);
}

Sketcher.prototype.simpleDraw = function (x, y, color, width, height) {
